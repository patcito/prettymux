...
export function ValidationLoop({
  ...
  const runValidate = validateAgents !== undefined && validateAgents.length > 0;
  return (
    <Loop id={`${idPrefix}:loop`} until={done} maxIterations={maxIterations} onMaxReached="return-last">
      <Sequence>
        ...
        {runValidate ? (
          <Task id={`${idPrefix}:validate`} output={validateOutputSchema} agent={validateAgents!} timeoutMs={1_800_000} heartbeatTimeoutMs={600_000}>
