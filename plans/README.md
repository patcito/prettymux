...
- **#1** — `CLAUDE.md` (entire file is Bun.serve / React / `bun:sqlite` / `bun test` guidance, none applicable). `index.ts` (30 bytes, `console.log("Hello via Bun!")`) and `tsconfig.json` tracked in git but unused. `package.json` + `bun.lock` are gitignored (local only; `package.json` pulls `smthrs`+`zod`, unused by the C app). Fix: replace CLAUDE.md with accurate C/GTK/meson guidance (build/test commands, module map: `workspace.c`, `session.c`, `socket_commands.c`, `app_actions.c`, `ghostty_terminal.c`, `workspace_strip.c`); delete or relocate `index.ts`/`tsconfig.json`.
